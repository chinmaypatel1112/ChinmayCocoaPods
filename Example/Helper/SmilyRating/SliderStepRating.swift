...
class SliderStep: UISlider {
    ...
    @IBInspectable var unselectedFont: UIFont = UIFont.init(name: "Poppins-Medium", size: 10) ?? UIFont.systemFont(ofSize: 12)
    @IBInspectable var selectedFont: UIFont = UIFont.init(name: "Poppins-Medium", size: 10) ?? UIFont.systemFont(ofSize: 12)
    ...
    @objc internal func drawLabels() {
        ...
        if let sl = _stepTickLabels {
            ...
            for index in 0..<ti.count {
                ...
                var rect = lbl.frame
                rect.origin.x = x
                rect.origin.y = bounds.midY - (bounds.size.height / 2) - rect.size.height + highlightedImageSize + 12
                lbl.frame = rect
                self.addSubview(lbl)
